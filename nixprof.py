...
def report(input: TextIO, tred, print_crit_path, print_avg_crit, print_sim_times, save_dot, save_chrome_trace, all, merge_into_pred, merge_into_succ, filter):
    ...
    drv_data = json.loads(subprocess.run(["nix", "--experimental-features", "nix-command flakes", "path-info", "--json", "--derivation"] + list(g), capture_output=True, check=True).stdout)
    for d in drv_data:
